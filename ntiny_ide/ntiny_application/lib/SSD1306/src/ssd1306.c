...
{ 
  ...
  while (no_of_commands) {
    ...
    // send command
    // -------------------------    
    SSD1306_Send_Command (command);
    // send arguments
    // -------------------------
    while (no_of_arguments--) {
      // send command
      SSD1306_Send_Command ((*commands++));
    }
    // decrement
    no_of_commands--;
  }
...
uint8_t SSD1306_Send_Command (uint8_t command)
{
  ...
  // TWI: start & SLAW
  // -------------------------
  SSD1306_Send_StartAndSLAW (SSD1306_ADDRESS);

  // send control byte
  // -------------------------    
  I2C_write (SSD1306_COMMAND,0); 
...
uint8_t SSD1306_NormalScreen (uint8_t address)
{
  ...
  // send command
  // -------------------------    
  SSD1306_Send_Command (SSD1306_DIS_NORMAL);
  // success
  return SSD1306_SUCCESS;
...
uint8_t SSD1306_InverseScreen (uint8_t address)
{
  ...
  SSD1306_Send_Command (SSD1306_DIS_INVERSE);
  // success
  return SSD1306_SUCCESS;
}
...
uint8_t SSD1306_UpdateScreen (uint8_t address)
{
  ...
  // -------------------------    
  I2C_write (SSD1306_DATA_STREAM,0);
  

  //  send cache memory lcd
  // -------------------------
  while (i < CACHE_SIZE_MEM - 1) {
    // send data
    I2C_write (cacheMemLcd[i],0);
    
    i++;
  }
  I2C_write (cacheMemLcd[i],1);
